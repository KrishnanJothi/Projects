...
def generate_eps(eps_length, action_size, task, actor_network, scale_vector):
...
def main():
    ...
    # Defining the optimizers for actor network
    ...
    for episode in range(0, 0 + no_of_episodes):
        ...
        Observations, Actions, Returns, status, Loss = generate_eps(episode_length, Dof, task, actor_network, scale_vec)
        if status == "Done":
            done_ = done_ + 1
        ...
        for k in range(len(Observations)):
            with tf.GradientTape() as tape:
                ...
                loss_value = CustomLossGaussian(Observations[k], Actions[k], Returns[k], Dof, actor_network,
                                                scale_vec)
